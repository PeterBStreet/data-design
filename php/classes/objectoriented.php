...
* autoload should be in classes;
	...
	/**
	 * Small Cross Section of a Medium like article
	 *
	 * This article can be considered a small example of what services like Medium store when articles are written and read on the Medium website. This can easily be extended to emulate more features of Medium.
...
* @var Uuid $userId
	 ...
	 * @throws \TypeError if $newTweetId is not a uuid or string
	 **/
	public function setuserId( $newTweetId) : void {
		try {
			$uuid = self::validateUuid($newTweetId);
		} catch(\InvalidArgumentException | \RangeException | \Exception | \TypeError $exception) {
			$exceptionType = get_class($exception);
